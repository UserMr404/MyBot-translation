...
#include "COCBot\functions\MOD\BBSpam.au3"
#include "COCBot\functions\MOD\CCSpam.au3"
#include "COCBot\functions\MOD\AttackCycle.au3"
...
; Initialize bot launch time for restart timer
...
Func FinalInitialization(Const $sAI)
	...
	If $g_iGuiMode = 0 Then
		SplashStep(GetTranslatedFileIni("MBR GUI Design - Loading", "Waiting_for_Remote_GUI", "Awaiting Remote GUI..."))
		SetDebugLog("Awaiting GUI Process...")
...
Func runBot()
	...
	If Not $g_bSkipInitialCheck Then
		; === CLAN GAMES ===
		$g_bClanGamesCompleted = False

		; === LAB AND PET UPGRADES ===
		; Update screen and display lab/pet upgrade information in GUI
		LabGuiDisplay()
		If _Sleep(50) Then Return
		...
		; === HERO AND BUILDING UPGRADES ===
		; Perform hero and building upgrade checks and actions
		UpgradeHeroes()
		If _Sleep(50) Then Return
		...
		; === MISC COLLECTION ===
		; Perform collection of achievements, free magic items and clan castle gold
		CollectAchievements()
		If _Sleep(50) Then Return
	...
	FirstCheck()
	While 1
		If Not $g_bRunState Then ExitLoop  ; Add immediate exit check for loop
		...
		If CheckBotRestart() Then
			SetLog("Bot restart triggered")
			_RestartBot()
		EndIf
		If Not $g_bIsClientSyncError Then ;ARCH:  was " And Not $g_bIsSearchLimit"
			...
			If IsSearchAttackEnabled() Then ; if attack is disabled skip reporting, requesting and boosting
				...
				; Check if army is ready after training and execute attack cycle if needed
				If $g_bIsFullArmywithHeroesAndSpells And $g_iCommandStop <> 0 And $g_iCommandStop <> 3 Then
					AttackCycle(False) ; False for Routine Cycle
				; if cycle ended early due to breaker pause/restart, exit outer loop
				If $g_bRestart Then ContinueLoop
				EndIf
			...
			Else
				...
				Else
					...
					If Not $bCloseGame Then
						...
						SetLog("Attack not planned and skipped. Waiting random " & StringFormat("%0.1f", $iWaitTime / 1000) & " seconds", $COLOR_WARNING)
...
Func _Idle() ;Sequence that runs until Full Army
	...
	While $g_bIsFullArmywithHeroesAndSpells = False
		...
		If $g_iCommandStop = -1 Then SetLog("Awaiting Full Army", $COLOR_ACTION)
		...
		If ($g_iCommandStop = 3 Or $g_iCommandStop = 0) And $g_bTrainEnabled = True Then
			...
			If ($g_bIsFullArmywithHeroesAndSpells = False) Then
				SetLog("Army camp not full, training continues", $COLOR_ACTION)
				$g_iCommandStop = 0
			EndIf
		EndIf
	...
	If IsSearchAttackEnabled() Then
		If IsSearchModeActive($DB) Or IsSearchModeActive($LB) Then
			...
			If $g_bDropTrophyEnable And Number($g_aiCurrentLoot[$eLootTrophy]) > Number($g_iDropTrophyMax) Then ;If current trophy above max trophy, try drop first
				...
				If _Sleep($DELAYATTACKMAIN1) Then Return
				Return ; return to runbot, refill army camps
			EndIf
		...
		EndIf
		Else
		SetLog("Attack not planned, skipped", $COLOR_WARNING)
		_RunFunction('DonateCC,Train')
		HeroStatus()
	EndIf
EndFunc   ;==>AttackMain
...
Func Attack() ;Selects which algorithm
	...
	ElseIf $g_iMatchMode = $DB And $g_aiAttackAlgorithm[$DB] = 2 Then
		If $g_bDebugSetLog Then SetDebugLog("Start SmartFarm Attack", $COLOR_ERROR)
	...
	Switch $action
		...
		Case "DonateCC"
			If $g_iActiveDonate And $g_bChkDonate Then
				...
				If _Sleep(50) = False Then checkMainScreen(False)
			EndIf
		...
		Case "DonateCC,Train"
			If $g_iActiveDonate And $g_bChkDonate Then
				If BalanceDonRec(True) Then DonateCC()
			EndIf
			If Not _Sleep(50) Then checkMainScreen(False)
		...
		Case "CheckBotRestart"
			CheckBotRestartCondition(False, True) ; Log the time remaining without restarting

		Case "RestartBot"
			_RestartBot() ; Force restart the bot
...
Func FirstCheck()
	...
	SetLog("Town Hall Level: " & $g_iTownHallLevel, $COLOR_INFO)
	...
	Else
		...
		SetLog("Town Hall Level detected as " & $g_iTownHallLevel, $COLOR_INFO)
		applyConfig()
		saveConfig()
	EndIf

	; === UPDATE TH LEVEL IN STATS GUI ===
	...
	If $g_iTownHallLevel > 6 Then
		If $g_aiHeroHallPos[1] <> -1 Then
			SetLog("Hero Hall Level: " & $g_aiHeroHallPos[2], $COLOR_INFO)
			...
			If StringInStr($sHeroHallInfo[1], "Hero") Then
				If $g_aiHeroHallPos[2] <> $sHeroHallInfo[2] Then
					...
					SetLog("Hero Hall Level detected as " & $g_aiHeroHallPos[2], $COLOR_INFO)
					applyConfig()
					saveConfig()
				EndIf
			Else
				If ImgLocateHeroHall() Then SetLog("Hero Hall found at (" & $g_aiHeroHallPos[0] & "," & $g_aiHeroHallPos[1] & "), Level: " & $g_aiHeroHallPos[2], $COLOR_DEBUG)
			EndIf
			ClearScreen()
		Else
			If Not HeroHallValuesCheck() Then
				SetLog("Please Check Hero Hall Values Now!", $COLOR_ERROR)
				SetLog("MBR cannot run without Hero Hall values - Locate it!", $COLOR_ERROR)
			EndIf
		EndIf
	EndIf
	...
	If $g_iCommandStop <> 0 And $g_iCommandStop <> 3 Then
		...
		If $g_bIsFullArmywithHeroesAndSpells Then
			...
			If $g_iCommandStop <> 0 And $g_iCommandStop <> 3 Then
				If Not $g_bRunState Then Return
				If SmartPause() Then Return
				AttackCycle(True) ; True for Initial Cycle
				If Not $g_bRunState Then Return
				If _Sleep(50) Then Return
			EndIf
		EndIf
...
Func _RestartBot()
	...
	SaveConfig()
	; Close Android if configured
	If $g_bAndroidCloseWithBot Then CloseAndroid("Bot Restart")
	; Terminate ADB instances
