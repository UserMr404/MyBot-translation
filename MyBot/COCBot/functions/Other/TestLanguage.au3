...
Func TestLanguage()
	...
	If getOcrLanguage($aDetectLang[0], $aDetectLang[1]) = "english" Then
		SetLog("Language Setting: English", $COLOR_INFO)
		Return True
	ElseIf Not ChangeLanguage() Then
		SetLog("Incorrect Language - Change To English", $COLOR_ERROR)
		btnStop()
	EndIf
EndFunc

Func ChangeLanguage()
	SetLog("Applying Language Settings", $COLOR_INFO)
	...
	For $i = 0 To 10 ; Checking OKAY Button continuously in 10sec
		If _ColorCheck(_GetPixelColor($aLanguageOkay[0], $aLanguageOkay[1], True), Hex($aLanguageOkay[2], 6), $aLanguageOkay[3]) Then
			...
			SetLog("   3. Click OKAY")
			SetLog("Please wait for loading Clash of Clans...")
			waitMainScreen()
			Return True
		EndIf
