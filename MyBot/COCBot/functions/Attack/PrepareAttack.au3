...
Func PrepareAttack($pMatchMode, $bRemaining = False) ;Assigns troops
	...
	If Not $bRemaining Then
		If $g_bPrepareAttackMeasurement Then
	...
	If $bRemaining Then
		SetLog("Checking remaining unused troops", $COLOR_INFO)
	Else
		SetLog("Preparing " & $g_asModeText[$pMatchMode] & " Attack", $COLOR_INFO)
	EndIf
	...
	Switch $ToUse
		Case $iTroopIndex ; the same as current castle/siege
			If $iTroopIndex <> $eCastle And $g_iSiegeLevel < $iMaxSiegeLevel Then
				$bNeedSwitch = True
				SetLog(GetTroopName($iTroopIndex) & " level " & $g_iSiegeLevel & " detected, looking for higher level")
			EndIf

		Case $eCastle, $eWallW, $eBattleB, $eStoneS, $eSiegeB, $eLogL, $eFlameF, $eBattleD, $eTroopL ; NOT the same as current castle/siege
			$bNeedSwitch = True
			SetLog(GetTroopName($iTroopIndex) & ($ToUse <> $eCastle ? " level " & $g_iSiegeLevel & " detected, looking for " : " detected, switching to ") & GetTroopName($ToUse))
		...
		Case "Any" ; use any siege
			If $iTroopIndex = $eCastle Or ($iTroopIndex <> $eCastle And $g_iSiegeLevel < $iMaxSiegeLevel) Then ; found Castle or a low level Siege
				$bNeedSwitch = True
				$bAnySiege = True
				SetLog(GetTroopName($iTroopIndex) & ($iTroopIndex = $eCastle ? " detected, looking for any siege machine" : " level " & $g_iSiegeLevel & " detected, looking for any higher siege machine"))
			EndIf
	EndSwitch
	...
	Else ; Index is a Hero/Siege/Castle/Spell
		...
		Else
			Switch $iTroopIndex
				...
				Case $eDragonDuke
					If (BitAND($g_aiAttackUseHeroes[$iMatchMode], $eHeroDragonDuke) = $eHeroDragonDuke) Then Return True
				...
				Case $eLSpell
					If $g_abAttackUseLightSpell[$iMatchMode] Or $g_bSmartZapEnable Or $g_aiAttackAlgorithm[$iMatchMode] = 1 Then Return True
				Case $eHSpell
					If $g_abAttackUseHealSpell[$iMatchMode] Or $g_aiAttackAlgorithm[$iMatchMode] = 1 Then Return True
				Case $eRSpell
					If $g_abAttackUseRageSpell[$iMatchMode] Or $g_aiAttackAlgorithm[$iMatchMode] = 1 Then Return True
				Case $eJSpell
					If $g_abAttackUseJumpSpell[$iMatchMode] Or $g_aiAttackAlgorithm[$iMatchMode] = 1 Then Return True
				Case $eFSpell
					If $g_abAttackUseFreezeSpell[$iMatchMode] Or $g_aiAttackAlgorithm[$iMatchMode] = 1 Then Return True
				Case $ePSpell
					If $g_abAttackUsePoisonSpell[$iMatchMode] Or $g_aiAttackAlgorithm[$iMatchMode] = 1 Then Return True
				Case $eESpell
					If $g_abAttackUseEarthquakeSpell[$iMatchMode] = 1 Or $g_bSmartZapEnable Or $g_aiAttackAlgorithm[$iMatchMode] = 1 Then Return True
				Case $eHaSpell
					If $g_abAttackUseHasteSpell[$iMatchMode] Or $g_aiAttackAlgorithm[$iMatchMode] = 1 Then Return True
				Case $eCSpell
					If $g_abAttackUseCloneSpell[$iMatchMode] Or $g_aiAttackAlgorithm[$iMatchMode] = 1 Then Return True
				Case $eISpell
					If $g_abAttackUseInvisibilitySpell[$iMatchMode] Or $g_aiAttackAlgorithm[$iMatchMode] = 1 Then Return True
				Case $eReSpell
					If $g_abAttackUseRecallSpell[$iMatchMode] Or $g_aiAttackAlgorithm[$iMatchMode] = 1 Then Return True
				Case $eRvSpell
					If $g_abAttackUseReviveSpell[$iMatchMode] Or $g_aiAttackAlgorithm[$iMatchMode] = 1 Then Return True
				Case $eTSpell
					If $g_abAttackUseTotemSpell[$iMatchMode] Or $g_aiAttackAlgorithm[$iMatchMode] = 1 Then Return True
				Case $eSkSpell
					If $g_abAttackUseSkeletonSpell[$iMatchMode] Or $g_aiAttackAlgorithm[$iMatchMode] = 1 Then Return True
				Case $eBtSpell
					If $g_abAttackUseBatSpell[$iMatchMode] Or $g_aiAttackAlgorithm[$iMatchMode] = 1 Then Return True
				Case $eOgSpell
					If $g_abAttackUseOvergrowthSpell[$iMatchMode] Or $g_aiAttackAlgorithm[$iMatchMode] = 1 Then Return True
				Case $eIBSpell
					If $g_abAttackUseIBSpell[$iMatchMode] Or $g_aiAttackAlgorithm[$iMatchMode] = 1 Then Return True
				Case Else
					Return False
			EndSwitch
			Return False
