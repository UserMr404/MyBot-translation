...
Func TestSmartFarm()
	...
	SetLog("Starting SmartFarm Attack Test", $COLOR_INFO)
	...
	SetLog("SmartFarm Attack Finished", $COLOR_INFO)
...
Func ChkSmartFarm($TypeResources = "All")
	...
	SetLog("Attacking " & UBound($BestSideToAttack) & " side(s): " & _ArrayToString($BestSideToAttack), $COLOR_INFO)
	SetLog("Calculation completed in " & Round(TimerDiff($hTimer) / 1000, 2) & " seconds", $COLOR_INFO)
	If Not $g_bRunState Then Return
...
Func AttackSmartFarm($Nside, $SIDESNAMES)
	...
	SetLog("Starting SmartFarm Attack", $COLOR_INFO)
	...
	EndIf
	SetLog("Located " & UBound($g_aiPixelMine) & " Gold Mines")
	SetLog("Located " & UBound($g_aiPixelElixir) & " Elixir Collectors")
	SetLog("Located " & UBound($g_aiPixelDarkElixir) & " Dark Elixir Drills")
	$g_aiNbrOfDetectedMines[$DB] += UBound($g_aiPixelMine)
	$g_aiNbrOfDetectedCollectors[$DB] += UBound($g_aiPixelElixir)
	$g_aiNbrOfDetectedDrills[$DB] += UBound($g_aiPixelDarkElixir)
	UpdateStats()
	...
	If $g_bCustomDropOrderEnable Then
 	...
 	Else
		Local $listInfoDeploy[56][5] = [[$eGole, $nbSides, 1, 1, 2] _
				...
				, [$eMeteorG, $nbSides, 1, 1, 2] _
				, [$eYeti, $nbSides, 1, 1, 2] _
				, [$eSYeti, $nbSides, 1, 1, 2] _
				...
				, [$eIceMini, $nbSides, 1, 1, 0] _
				, [$eSMini, $nbSides, 1, 1, 0] _
				, [$eIceWiz, $nbSides, 1, 1, 0] _
				...
				, [$eHunt, $nbSides, 1, 1, 0] _
 				...
 				, [$eGGiant, $nbSides, 1, 1, $g_iSlotsGiants] _
 				, [$eAzureDragon, $nbSides, 1, 1, 0] _
 				, [$eFirecracker, $nbSides, 1, 1, 0] _
	...
	LaunchTroopSmartFarm($listInfoDeploy, $g_iClanCastleSlot, $g_iKingSlot, $g_iQueenSlot, $g_iPrinceSlot, $g_iWardenSlot, $g_iChampionSlot, $g_iDragonDukeSlot, $SIDESNAMES)
	...
	SetLog("Dropping leftover troops", $COLOR_INFO)
	For $x = 0 To 1
		If PrepareAttack($g_iMatchMode, True) = 0 Then
			If $g_bDebugSetLog Then SetDebugLog("No usable troops left, exiting", $COLOR_DEBUG)
	...
	SetLog("Finished attacking, waiting for battle end")

EndFunc   ;==>AttackSmartFarm

Func LaunchTroopSmartFarm($listInfoDeploy, $iCC, $iKing, $iQueen, $iPrince, $iWarden, $iChampion, $iDragonDuke = -1, $SIDESNAMES = "TR|TL|BR|BL")
					...
					For $j = 0 To UBound($listInfoDeployTroopPixel) - 1
						...
						If (IsString($infoTroopListArrPixel[0]) And ($infoTroopListArrPixel[0] = "CC" Or $infoTroopListArrPixel[0] = "HEROES")) Then
							...
							ElseIf ($g_bIsHeroesDropped = False And $infoTroopListArrPixel[0] = "HEROES" And $i = $numberSidesDropTroop - 1) Then
								dropHeroes($pixelRandomDrop[0], $pixelRandomDrop[1], $iKing, $iQueen, $iPrince, $iWarden, $iChampion, $iDragonDuke)
