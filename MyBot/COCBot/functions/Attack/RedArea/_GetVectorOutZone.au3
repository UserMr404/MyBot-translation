...
Func _GetVectorOutZone($eVectorType)
	...
	For $i = 0 To $iSteps
		...
		If $pixel[0] < 0 Then $pixel[0] = 0
		If $pixel[0] > $g_iGAME_WIDTH Then $pixel[0] = $g_iGAME_WIDTH
		If $pixel[1] < 0 Then $pixel[1] = 0
		If $pixel[1] > $g_iGAME_HEIGHT Then $pixel[1] = $g_iGAME_HEIGHT
