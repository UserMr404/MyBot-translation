...
Func ReturnHome($TakeSS = 1, $GoldChangeCheck = True) ;Return main screen
	...
	$g_aHeroesTimerActivation[$eHeroDragonDukeIndex] = 0
	
	; Reset battle type flags
	$g_bIsRankedBattleActive = False
	$g_bIsRevengeBattleActive = False
	...
	If Not (IsReturnHomeBattlePage(True, False)) Then ; check if battle is already over
		...
		Next
		If $bret Then
			SetLog("Have strange problem Couldn't Click Okay to Confirm surrender, Restarting Clash of Clans", $COLOR_ERROR)
			CloseCoC(True)
			Return
		EndIf
	Else
	...
	While 1
		...
		If _CheckPixel($aEndFightSceneAvl, $g_bCapturePixel) Or ($i > 10 And IsReturnHomeBattlePage(True)) Then ; check for the gold ribbon or fallback after 10 tries
			If IsReturnHomeBattlePage(True) Then
				ClickP($aReturnHomeButton, 1, 120, "#0101") ;Click Return Home Button
				If _Sleep(500) Then Return
				If IsReturnHomeBattlePage(True) Then ; if still on page, try fallback position
					Local $aReturnHomeButtonFallback[4] = [Random(425, 435), Random(570 + $g_iMidOffsetY, 580 + $g_iMidOffsetY), 0x6CBB1F, 15]
					ClickP($aReturnHomeButtonFallback, 1, 120, "#0101") ;Click Return Home Button Fallback
				EndIf
