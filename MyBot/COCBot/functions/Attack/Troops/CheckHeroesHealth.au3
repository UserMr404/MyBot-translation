...
Func CheckHeroesHealth()
	...
	If $g_bCheckKingPower Or $g_bCheckQueenPower Or $g_bCheckPrincePower Or $g_bCheckWardenPower Or $g_bCheckChampionPower Or $g_bCheckDragonDukePower Then
		...
		If $g_iKingSlot >= 11 Or $g_iQueenSlot >= 11 Or $g_iPrinceSlot >= 11 Or $g_iWardenSlot >= 11 Or $g_iChampionSlot >= 11 Or $g_iDragonDukeSlot >= 11 Then
			If Not $g_bDraggedAttackBar Then DragAttackBar($g_iTotalAttackSlot, False)         ; drag forward
		ElseIf $g_iKingSlot >= 0 And $g_iQueenSlot >= 0 And $g_iPrinceSlot >= 0 And $g_iWardenSlot >= 0 And $g_iChampionSlot >= 0 And $g_iDragonDukeSlot >= 0 And ($g_iKingSlot < $g_iTotalAttackSlot - 10 Or $g_iQueenSlot < $g_iTotalAttackSlot - 10 Or _
				$g_iPrinceSlot < $g_iTotalAttackSlot - 10 Or $g_iWardenSlot < $g_iTotalAttackSlot - 10 Or $g_iChampionSlot < $g_iTotalAttackSlot - 10 Or $g_iDragonDukeSlot < $g_iTotalAttackSlot - 10) Then
		...
		If $g_iActivateDragonDuke = 0 Or $g_iActivateDragonDuke = 2 And ($g_aHeroesTimerActivation[$eHeroDragonDukeIndex] = 0 Or __TimerDiff($g_aHeroesTimerActivation[$eHeroDragonDukeIndex]) > $DELAYCHECKHEROESHEALTH) Then
			If $g_bCheckDragonDukePower Then
				Local $asDragonDukeResult = decodeSingleCoord(FindImageInPlace2("Dragon Duke", $g_sImgDragonDukeBar, 0, 570 + $g_iBottomOffsetY, 858, 638 + $g_iBottomOffsetY, True)) ; Looking for Dragon Duke
				If IsArray($asDragonDukeResult) And UBound($asDragonDukeResult) = 2 Then
					Local $aDragonDukeHealthCopy = $aDragonDukeHealth
					$aDragonDukeHealthCopy[0] = $asDragonDukeResult[0] - $aDragonDukeHealthCopy[4]
					Local $DragonDukePixelColor = _GetPixelColor($aDragonDukeHealthCopy[0], $aDragonDukeHealthCopy[1], $g_bCapturePixel)
					If $g_bDebugSetLog Then SetDebugLog("Dragon Duke _GetPixelColor(" & $aDragonDukeHealthCopy[0] & "," & $aDragonDukeHealthCopy[1] & "): " & $DragonDukePixelColor, $COLOR_DEBUG)
