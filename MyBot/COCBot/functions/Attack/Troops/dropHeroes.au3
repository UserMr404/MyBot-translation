...
; Syntax ........: dropHeroes($x, $y, $iKingSlot = -1, $iQueenSlot = -1, $iPrinceSlot = -1, $iWardenSlot = -1, $iChampionSlot = -1, $iDragonDukeSlot = -1)
...
Func dropHeroes($iX, $iY, $iKingSlotNumber = -1, $iQueenSlotNumber = -1, $iPrinceSlotNumber = -1, $iWardenSlotNumber = -1, $iChampionSlotNumber = -1, $iDragonDukeSlotNumber = -1) ;Drops for All Heroes
	If $g_bDebugSetLog Then SetDebugLog("dropHeroes $iKingSlotNumber " & $iKingSlotNumber & " $iQueenSlotNumber " & $iQueenSlotNumber & " $iPrinceSlotNumber " & $iPrinceSlotNumber & " $iWardenSlotNumber " & $iWardenSlotNumber & " $iChampionSlotNumber " & $iChampionSlotNumber & " $iDragonDukeSlotNumber " & $iDragonDukeSlotNumber & " matchmode " & $g_iMatchMode, $COLOR_DEBUG)
	...
	If $iDragonDukeSlotNumber <> -1 And (($g_iMatchMode <> $DB And $g_iMatchMode <> $LB) Or BitAND($g_aiAttackUseHeroes[$g_iMatchMode], $eHeroDragonDuke) = $eHeroDragonDuke) Then $bDropDragonDuke = True

	; Ensure hero order array is valid; fallback to default order if not
	Local $aHeroOrder[6] = [0, 1, 2, 3, 4, 5]
	If IsArray($g_aiHeroDeployOrder) Then $aHeroOrder = $g_aiHeroDeployOrder
	For $i = 0 To UBound($aHeroOrder) - 1
		Switch $aHeroOrder[$i]
			Case 0
			...
			Case 5
				If $g_bDebugSetLog Then SetDebugLog("drop DRAGON DUKE = " & $bDropDragonDuke, $COLOR_DEBUG)
				DragonDukeDrop($iX, $iY, $iDragonDukeSlotNumber, $bDropDragonDuke)
		EndSwitch
	Next
...
Func QueenDrop($iX, $iY, $iQueenSlotNumber = -1, $bDropQueen = False)
	If $bDropQueen Then
		SetLog("Dropping Queen at " & $iX & ", " & $iY)
		SelectDropTroop($iQueenSlotNumber, 1, Default, False)
		If _Sleep($DELAYDROPHEROES2) Then Return
		AttackClick($iX + Random(0, 3, 1), $iY + Random(0, 3, 1), 1, 50, 0, "#0095")
...
Func PrinceDrop($iX, $iY, $iPrinceSlotNumber = 1, $bDropPrince = False)
	If $bDropPrince Then
		SetLog("Dropping Prince at " & $iX & ", " & $iY)
...
Func WardenDrop($iX, $iY, $iWardenSlotNumber = -1, $bDropWarden = False)
	If $bDropWarden Then
		SetLog("Dropping Grand Warden at " & $iX & ", " & $iY)
		SelectDropTroop($iWardenSlotNumber, 1, Default, False)
		If _Sleep($DELAYDROPHEROES2) Then Return
		AttackClick($iX + Random(0, 3, 1), $iY + Random(0, 3, 1), 1, 50, 0, "#x999")
...
Func ChampionDrop($iX, $iY, $iChampionSlotNumber = -1, $bDropChampion = False)
	If $bDropChampion Then
		SetLog("Dropping Royal Champion at " & $iX & ", " & $iY)
...
Func DragonDukeDrop($iX, $iY, $iDragonDukeSlotNumber = -1, $bDropDragonDuke = False)
	If $bDropDragonDuke Then
		SetLog("Dropping Dragon Duke at " & $iX & ", " & $iY)
		SelectDropTroop($iDragonDukeSlotNumber, 1, Default, False)
		If _Sleep($DELAYDROPHEROES2) Then Return
		AttackClick($iX + Random(0, 3, 1), $iY + Random(0, 3, 1), 1, 50, 0, "#x999")
		If Not $g_bDropDragonDuke Then ; check global flag, only begin hero health check on 1st hero drop as flag is reset to false after activation
		...
		EndIf
		$g_bDropDragonDuke = True ; Set global flag hero dropped
		$g_aHeroesTimerActivation[$eHeroDragonDukeIndex] = __TimerInit() ; initialize fixed activation timer
		If _Sleep($DELAYDROPHEROES1) Then Return
	EndIf
	If _Sleep($DELAYDROPHEROES1) Then Return
EndFunc   ;==>DragonDukeDrop
