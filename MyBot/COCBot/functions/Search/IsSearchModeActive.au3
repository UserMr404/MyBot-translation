...
Func IsSearchModeActive($g_iMatchMode, $bDontCheckHeroes = False, $bNoLog = False)
	...
	Local $currentTrophies = $g_aiCurrentLoot[$eLootTrophy]
	...
	Local $checkTrophies = Int($currentTrophies) >= Int($g_aiSearchTrophiesMin[$g_iMatchMode]) And Int($currentTrophies) <= Int($g_aiSearchTrophiesMax[$g_iMatchMode]) And $g_abSearchTrophiesEnable[$g_iMatchMode]
	...
	If $bCheckHeroes And $g_bCheckSpells And $bcheckSiege Then ;If $bCheckHeroes Then
		If ($checkSearches Or $g_abSearchSearchesEnable[$g_iMatchMode] = False) And ($checkTrophies Or $g_abSearchTrophiesEnable[$g_iMatchMode] = False) And ($checkArmyCamps Or $g_abSearchCampsEnable[$g_iMatchMode] = False) Then
			If $g_bDebugSetLog And Not $bNoLog Then SetLog($g_asModeText[$g_iMatchMode] & " active! ($checkSearches=" & $checkSearches & _
					",$checkTrophies=" & $checkTrophies & _
		...
		Else
			If $g_bDebugSetLog And Not $bNoLog Then
				...
				Local $txtTrophies = "Fail"
				If $checkTrophies Then $txtTrophies = "Success"
				...
				If $g_abSearchTrophiesEnable[$g_iMatchMode] Then SetLog("trophies range: " & $g_aiSearchTrophiesMin[$g_iMatchMode] & "-" & $g_aiSearchTrophiesMax[$g_iMatchMode] & "  actual value: " & $currentTrophies & " | " & $txtTrophies, $COLOR_INFO)
