; #FUNCTION# ====================================================================================================================
; Name ..........: VillageSearch
; Description ...: Searches for a village that meets conditions
...
Func _VillageSearch() ;Control for searching a village that meets conditions
	...
	If $g_bIsRevengeBattleActive Or $g_bIsRankedBattleActive Then
		If $g_abAttackTypeEnable[$LB] Then
			$g_iMatchMode = $LB
		ElseIf $g_abAttackTypeEnable[$DB] Then
			$g_iMatchMode = $DB
		EndIf
		_Sleep(1500) ; wait a bit for the battle screen to load
		Return True
	EndIf
	...
	If Not ($g_bIsSearchLimit) Then
		If Not $g_bFastSearchMode Then SetLogCentered("=", "=", $COLOR_INFO)
	EndIf
	...
	If Not ($g_bIsSearchLimit) Then
		If Not $g_bFastSearchMode Then SetLogCentered("=", "=", $COLOR_INFO)
	Else
		If Not $g_bFastSearchMode Then SetLogCentered(" Restart To Search ", Default, $COLOR_INFO)
	EndIf
	...
	While 1 ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;### Main Search Loop ###;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
		...
		; cleanup some vars used by imgloc just in case. used in TH and DeadBase ( imgloc functions)
		ResetTHsearch()
		...
		Local $iResourceRetries = 0
		While $iResourceRetries < 3 And ($g_iSearchGold = 0 And $g_iSearchElixir = 0) ; Assume 0 means failed detection
			...
			$iResourceRetries += 1
		WEnd
		If $iResourceRetries >= 3 Then SetLog("Resource detection failed after retries", $COLOR_ERROR)
		...
		If (IsWeakBaseActive($DB) And $dbBase And ($match[$DB] Or $g_abFilterMeetOneConditionEnable[$DB])) Or _
			...
			For $try = 1 To $maxTry ; check twice to be sure due to walking heroes
				;let try to reduce weakbase time
				If ($g_iSearchTH <> "-") Then
				...
				If $bIsWeak And $try = 1 And Not $g_bFastSearchMode Then
					ResumeAndroid()
		...
		; ----------------- WRITE LOG VILLAGE FOUND AND ASSIGN VALUE AT $g_iMatchMode and exitloop  IF CONDITIONS MEET ---------------------------
		If $match[$DB] And $dbBase Then
			If Not $g_bFastSearchMode Then
		...
		If Not ($logwrited) Then
			If Not $g_bFastSearchMode Then SetLog($GetResourcesTXT, $COLOR_BLACK, "Lucida Console", 7.5)
		EndIf

		If $g_bSearchRestartPickupHero And Not $g_bFastSearchMode Then
			For $i = 0 To $eHeroSlots - 1 ; check slots
				...
				If Not $g_bFastSearchMode Then SetDebugLog($g_asHeroNames[$g_aiCmbCustomHeroOrder[$i]] & " will be ready in " & $iTimeTillHeroHealed & " seconds")
				If $iTimeTillHeroHealed <= 0 Then
					...
					ExitLoop ; found 1 Hero is ready, skip checking other heroes
				EndIf
			Next
		EndIf
		...
		If Not $g_bFastSearchMode Then SetDebugLog("Detections made in : " & $iDetectionTimerSetLog & "ms", $COLOR_DEBUG)
		...
		While $i < 100
			...
			If (_ColorCheck(_GetPixelColor($NextBtn[0], $NextBtn[1]), Hex($NextBtn[2], 6), $NextBtn[3])) And IsAttackPage(False) Then
				...
				;Click(755, 572, 1, 120, "#0155") ;Click Next
				;Click(755 + Random(1, 5, 1),572 + Random(1, 5, 1)) ;
				;Click(755 + Random(1, 5, 1), 572 + Random(1, 5, 1), 1, 120, "#0155") ;Click Next
				Click(755 + Random(1, 5, 1), 572 + Random(1, 5, 1), 1, 120, "#0155") ;Click Next
				ExitLoop
			Else
			...
			If $i >= 99 Or isProblemAffect() Or (Mod($i, 10) = 0 And checkObstacles_Network(False, False)) Then ; if we can't find the next button or there is an error, then restart
				...
				Else
					SetLog("Have strange problem couldn't locate Next button, Restarting Clash of Clans and Bot...", $COLOR_ERROR)
...
Func WriteLogVillageSearch($x)
	;this function writes in BOT LOG the values setting for each attack mode ($DB,$LB)
	;example
	...
	If Not ($g_bIsSearchLimit) And $g_bDebugSetLog Then
		...
		If $g_abSearchTrophiesEnable[$x] Then SetLog("- Search trophies range " & $g_aiSearchTrophiesMin[$x] & " - " & $g_aiSearchTrophiesMax[$x], $COLOR_INFO)
