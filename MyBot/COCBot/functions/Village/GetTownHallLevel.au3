...
Func GetTownHallLevel($bFirstTime = False)
	...
	If $aTHInfo[0] > 1 Then
		...
		If $aTHInfo[2] <> "" Then
			...
			SetLog("Town Hall Detected: (" & $g_aiTownHallPos[0] & "," & $g_aiTownHallPos[1] & "), Level: " & $g_iTownHallLevel, $COLOR_SUCCESS)
			saveConfig()
		Else
			SetLog("Town Hall Level not detected via text - Using Fallback: Setting to Level 17", $COLOR_WARNING)
			$g_iTownHallLevel = 17
			saveConfig()
		EndIf
	Else
		SetLog("Your Town Hall Level was not found (no building info)! Using fallback: setting to level 17.", $COLOR_WARNING)
		$g_iTownHallLevel = 17
		saveConfig()
	EndIf
