...
Func isElixirFull()
	...
	If IsArray($aElixirFull) Then
		SetLog("Elixir Storages: Full", $COLOR_SUCCESS)
	...
	ElseIf $g_abFullStorage[$eLootElixir] Then
		If Number($g_aiCurrentLoot[$eLootElixir]) >= Number($g_aiResumeAttackLoot[$eLootElixir]) Then
			SetLog("Elixir Storages Almost Full: " & $g_aiCurrentLoot[$eLootElixir], $COLOR_SUCCESS)
			$g_abFullStorage[$eLootElixir] = True
		Else
			SetLog("Resume Normal Mode: Elixir < " & $g_aiResumeAttackLoot[$eLootElixir], $COLOR_SUCCESS)
