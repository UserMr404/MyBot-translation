...
Func _AutoUpgrade()
	...
	While 1
		...
		If ($g_iFreeBuilderCount - ($g_bAutoUpgradeWallsEnable And $g_bUpgradeWallSaveBuilder ? 1 : 0) - ReservedBuildersForHeroes()) <= 0 Then
			SetLog("No builder available, skipping auto upgrade")
			ExitLoop
		EndIf
		...
		If IsArray($aTmpCoord) And UBound($aTmpCoord) > 0 Then
			...
			Else
				SetLog("Not enough resource, looking next", $COLOR_DEBUG)
				ContinueLoop
			EndIf
		Else
			SetLog("No upgrade available, exiting auto upgrade", $COLOR_DEBUG)
			ExitLoop
		EndIf
		...
		If QuickMIS("NX", $g_sImgAUpgradeObst, 180, $aTmpCoord[0][2] - 15, 480, $aTmpCoord[0][2] + 15) <> "none" Then
			SetLog("This is a new building or equipment, looking next", $COLOR_WARNING)
			ContinueLoop
		EndIf
		...
		If $IsElix Or $IsDark Then
			...
			If IsArray($aHeroTempl) And UBound($aHeroTempl) > 0 Then
				Switch $aHeroTempl[0][0]
					...
					Case "Duke"
						$IHeroName = "Dragon Duke"
						$bHeroCoordButton = $bXcoords[4] + 80 ; After scroll, Duke lands on the last visible slot
				EndSwitch
			EndIf
		EndIf
		...
		Else
			...
			If Not (IsArray($aUpgradeButton) And UBound($aUpgradeButton, 1) = 2) Then
				SetLog("No upgrade found, moving to the next option...", $COLOR_DEBUG)
				ContinueLoop
			EndIf
		...
		If $bHeroSystem Then
			; Dragon Duke is the 6th hero - requires scrolling the Hero Hall bar to reveal it
			If $g_aUpgradeNameLevel[1] = "Dragon Duke" Then
				SetLog("Scrolling Hero Hall to reveal Dragon Duke slot...", $COLOR_INFO)
			...
			Switch $g_aUpgradeNameLevel[1]
				...
				Case "Dragon Duke"
					Local $g_iDragonDukeLevelOCR = getYellowLevel(525, 116)
					Local $g_aUpgradeNameLevelTemp2[3] = [$g_aUpgradeNameLevel[0], $g_aUpgradeNameLevel[1], Number($g_iDragonDukeLevelOCR - 1)]
			EndSwitch
		...
		; initiate a False boolean, that firstly says that there is no sufficient resource to launch upgrade
		Local $bSufficientResourceToUpgrade = False
		...
		Switch $g_aUpgradeResourceCostDuration[0]
			Case "Gold"
				If $g_aiCurrentLoot[$eLootGold] >= ($g_aUpgradeResourceCostDuration[1] + $g_iTxtSmartMinGold) Then $bSufficientResourceToUpgrade = True
			Case "Elixir"
				If $g_aiCurrentLoot[$eLootElixir] >= ($g_aUpgradeResourceCostDuration[1] + $g_iTxtSmartMinElixir) Then $bSufficientResourceToUpgrade = True
			Case "Dark Elixir"
				If $g_aiCurrentLoot[$eLootDarkElixir] >= ($g_aUpgradeResourceCostDuration[1] + $g_iTxtSmartMinDark) Then $bSufficientResourceToUpgrade = True
		EndSwitch
		; if boolean still False, we can't launch upgrade, exiting...
		If Not $bSufficientResourceToUpgrade Then
