...
Func UpgradeWall()
	...
	If $g_bAutoUpgradeWallsEnable = True Then
		...
		SetLog("Checking Wall Upgrades", $COLOR_INFO)
		...
		If $g_iFreeBuilderCount > 0 Then
						...
						If $MinWallGold Then
							...
							If imglocCheckWall() Then
								If Not UpgradeWallGold($iWallCost) Then
									SetLog("Upgrade with gold failed, skipping")
									Return
								EndIf
							...
							EndIf
						Else
							SetLog("Gold below minimum, skipping upgrade")
						EndIf
					Case 1
						If $MinWallElixir Then
							SetLog("Upgrading wall using elixir")
							If imglocCheckWall() Then
								If Not UpgradeWallElixir($iWallCost) Then
									SetLog("Upgrade with elixir failed, skipping")
									Return
								EndIf
							ElseIf SwitchToNextWallLevel() Then
								SetLog("No more walls of current level, switching to next", $COLOR_ACTION)
							...
							EndIf
						Else
							SetLog("Elixir below minimum, skipping upgrade")
						EndIf
					Case 2
						If $MinWallElixir Then
							SetLog("Upgrading wall using elixir")
							If imglocCheckWall() Then
								If Not UpgradeWallElixir($iWallCost) Then
									SetLog("Upgrade with elixir failed, attempting upgrade with gold")
									If Not UpgradeWallGold($iWallCost) Then
										SetLog("Upgrade with gold failed, skipping")
										Return
									EndIf
							...
							EndIf
						Else
							SetLog("Elixir below minimum, attempting upgrade with gold")
							If $MinWallGold Then
								If imglocCheckWall() Then
									If Not UpgradeWallGold($iWallCost) Then
										SetLog("Upgrade with gold failed, skipping")
			...
			WEnd
		Else
			SetLog("No free builder, wall upgrades skipped")
		EndIf
	EndIf
...
Func UpgradeWallGold($iWallCost = $g_iWallCost)
	...
	If _ColorCheck(_GetPixelColor(800, 88 + $g_iMidOffsetY, True), Hex(0xF38E8E, 6), 20) Then ; wall upgrade window red x
		If isNoUpgradeLoot(False) = True Then
			SetLog("Upgrade stopped due to insufficient resources", $COLOR_DEBUG)
			Return False
		EndIf
		...
		If isGemOpen(True) Then
			SetLog("Upgrade stopped due to insufficient resources", $COLOR_DEBUG)
			Return False
		ElseIf _ColorCheck(_GetPixelColor(800, 88 + $g_iMidOffsetY, True), Hex(0xF38E8E, 6), 20) Then ; wall upgrade window red x, didnt closed on upgrade click, so not able to upgrade
			CloseWindow()
			SetLog("Unable to upgrade", $COLOR_DEBUG)
			Return False
		Else
	...
	ClearScreen()
	SetLog("No upgrade gold button found", $COLOR_ERROR)
...
Func UpgradeWallElixir($iWallCost)
	...
	ClearScreen()
	SetLog("No upgrade elixir button found", $COLOR_ERROR)
	...
	Switch $g_iTownHallLevel
		Case 5 To 8 ;Start at Townhall 5 because any Wall Level below 4 is not supported anyways
			...
			If $g_iTownHallLevel < $g_iCmbUpgradeWallsLevel + 4 Then
				SetLog("Skipping wall upgrade, insufficient Town Hall Level", $COLOR_WARNING)
				Return True
			EndIf
				...
				Case 2 ; Using gold and elixir
					If $g_aiCurrentLoot[$eLootElixir] - ($iUpgradesNeedElixir + $iWallCost + Number($g_iUpgradeWallMinElixir)) < 0 Then
						...
						Else
							...
							SetLog("Using gold only for wall upgrade")
					...
					Else
						If $g_aiCurrentLoot[$eLootGold] - ($iUpgradesNeedGold + $iWallCost + Number($g_iUpgradeWallMinGold)) < 0 Then
							...
							SetLog("Using elixir only for wall upgrade")
