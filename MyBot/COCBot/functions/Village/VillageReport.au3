...
Func VillageReport($bBypass = False, $bSuppressLog = False)
	...
	; === BUILDER BASE SPAM ===
	; Execute Builder Base spam cycle if enabled
	If $g_bBBSpamEnabled And Not $g_bBBSpamRunning Then
		BBSpamCycle()
	EndIf

	; === CLAN CASTLE DONATION SPAM ===
	; Execute CC Donation spam cycle if enabled
	If $g_bCCSpamEnabled And Not $g_bCCSpamRunning Then
		CCSpamCycle()
	EndIf
