...
Func isGoldFull()
	...
	If IsArray($aGoldFull) Then
		SetLog("Gold Storages: Full", $COLOR_SUCCESS)
	...
	ElseIf $g_abFullStorage[$eLootGold] Then
		If Number($g_aiCurrentLoot[$eLootGold]) >= Number($g_aiResumeAttackLoot[$eLootGold]) Then
			SetLog("Gold Storages Almost Full: " & $g_aiCurrentLoot[$eLootGold], $COLOR_SUCCESS)
			$g_abFullStorage[$eLootGold] = True
		Else
			SetLog("Resume Normal Mode: Gold < " & $g_aiResumeAttackLoot[$eLootGold], $COLOR_SUCCESS)
