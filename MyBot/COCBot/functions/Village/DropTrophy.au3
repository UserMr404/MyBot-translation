...
Func DropTrophy()
	...
	If $g_bDropTrophyEnable Then
		...
		If $g_bDropTrophyUseHeroes And $g_iHeroAvailable > 0 Then
			SetDebugLog("Drop Trophy Found Hero BK|AQ|MP|GW|RC|DD: " & BitOR($g_iHeroAvailable, $eHeroKing) & "|" & BitOR($g_iHeroAvailable, $eHeroQueen) & "|" & BitOR($g_iHeroAvailable, $eHeroPrince) & "|" & _
					BitOR($g_iHeroAvailable, $eHeroWarden) & "|" & BitOR($g_iHeroAvailable, $eHeroChampion) & "|" & BitOR($g_iHeroAvailable, $eHeroDragonDuke), $COLOR_DEBUG)
			$bHaveTroops = True
		EndIf
		...
		While Number($g_aiCurrentLoot[$eLootTrophy]) > Number($g_iDropTrophyMaxNeedCheck)
			...
			If Number($g_aiCurrentLoot[$eLootTrophy]) > Number($g_iDropTrophyMaxNeedCheck) Then
				...
				If $g_bDropTrophyUseHeroes Then
					...
					If ($g_iQueenSlot <> -1 Or $g_iKingSlot <> -1 Or $g_iPrinceSlot <> -1 Or $g_iWardenSlot <> -1 Or $g_iChampionSlot <> -1 Or $g_iDragonDukeSlot <> -1) Then
						Local $sHeroPriority
						Switch $g_iDropTrophyHeroesPriority
							Case 0
								$sHeroPriority = "QKPWCD"
							Case 1
								$sHeroPriority = "QWPKCD"
							Case 2
								$sHeroPriority = "KPQWCD"
							Case 3
								$sHeroPriority = "PKWQCD"
							Case 4
								$sHeroPriority = "WKQPCD"
							Case 5
								$sHeroPriority = "WPQKCD"
							Case 6
								$sHeroPriority = "CWQKPD"
							Case 7
								$sHeroPriority = "CQWPKD"
						EndSwitch
						...
						For $i = 1 To 6
							...
							Switch $t
								...
								Case "C"
									...
									EndIf
								Case "D"
									If $g_iDragonDukeSlot <> -1 Then
										SetTrophyLoss()
										SetLog("Deploying Dragon Duke", $COLOR_INFO)
										SelectDropTroop($g_iDragonDukeSlot)
										If _Sleep($DELAYDROPTROPHY1) Then ExitLoop
										Click($aRandomEdge[$iRandomXY][0], $aRandomEdge[$iRandomXY][1], 1, 120, "#0000") ;Drop Dragon Duke
										If _Sleep($DELAYDROPTROPHYHERO) Then ExitLoop
										SelectDropTroop($g_iDragonDukeSlot) ;If Dragon Duke was not activated: Boost Dragon Duke before EndBattle to restore some health
										ReturnfromDropTrophies()
										If _Sleep($DELAYDROPTROPHY1) Then ExitLoop
										ExitLoop
									EndIf
					...
					EndIf
				EndIf
				If ($g_iQueenSlot = -1 And $g_iKingSlot = -1 And $g_iPrinceSlot = -1 And $g_iWardenSlot = -1 And $g_iChampionSlot = -1 And $g_iDragonDukeSlot = -1) Or $g_bDropTrophyUseHeroes = 0 Then
					$aRandomEdge = $g_aaiEdgeDropPoints[Round(Random(0, 3))]
					$iRandomXY = Round(Random(0, 4))
