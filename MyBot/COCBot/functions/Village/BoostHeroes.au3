...
Func BoostDragonDuke()
	; Verifying existent Variables to run this routine
	If AllowBoosting("Dragon Duke", $g_iCmbBoostDragonDuke) = False Then Return
	...
	If BoostStructure("Dragon Duke", "Duke", $g_aiHeroHallPos, $g_iCmbBoostDragonDuke, $g_hCmbBoostDragonDuke) Then $g_aiHeroBoost[$eHeroDragonDukeIndex] = _NowCalc()
	$g_aiTimeTrain[2] = 0 ; reset Heroes remaining time

	If _Sleep($DELAYBOOSTBARRACKS3) Then Return
	checkMainScreen(False) ; Check for errors during function
EndFunc   ;==>BoostDragonDuke
