...
Local $bXcoords[6] = [65, 218, 371, 523, 676, 718]
...
Func FinishTimeCalculation($bHero = "")
	Switch $bHero
		...
		Case "Champion"
			...
			If _Sleep($DELAYUPGRADEHERO3) Then Return
		Case "Duke"
			ClickDrag(400, 300 + $g_iMidOffsetY, 247, 300 + $g_iMidOffsetY, 50) ; Swipe left 153px to reveal Dragon Duke
			If _Sleep(800) Then Return ; Wait for scroll animation
			Click($bXcoords[4] + 115, 232 + $g_iMidOffsetY)
			If _Sleep($DELAYUPGRADEHERO3) Then Return ; Wait for window to open
			Local $g_aUpgradeDuration = getHeroUpgradeTime(715, 544 + $g_iMidOffsetY) ; get duration
			Local $HeroUpgradeTime = ConvertOCRTime("TimeToFinish", $g_aUpgradeDuration, False)
			...
			EndIf
			SetDebugLog("Upgrade Time OCR : " & $g_aUpgradeDuration)
			$g_aiHeroUpgradeFinishDate[5] = _DateAdd('n', Ceiling($HeroUpgradeTime), _NowCalc())
			SetLog($bHero & " Upgrade Finishes @ " & $g_aiHeroUpgradeFinishDate[5], $COLOR_SUCCESS)
			If _Sleep(200) Then Return
			CloseWindow2()
			If _Sleep($DELAYUPGRADEHERO3) Then Return
	EndSwitch
...
Func NeededResources($bHero = "")
	Switch $bHero
		Case "King"
			...
			SetLog($bHero & " Upgrade Requires " & _NumberFormat($g_aiHeroNeededResource[0]) & " Dark Elixir", $COLOR_INFO)
			If _Sleep(200) Then Return
			CloseWindow2()
		Case "Queen"
			Click($bXcoords[1] + 115, 232 + $g_iMidOffsetY)
			...
			SetLog($bHero & " Upgrade Requires " & _NumberFormat($g_aiHeroNeededResource[1]) & " Dark Elixir", $COLOR_INFO)
			If _Sleep(200) Then Return
			CloseWindow2()
		Case "Prince"
			Click($bXcoords[2] + 115, 232 + $g_iMidOffsetY)
			...
			SetLog($bHero & " Upgrade Requires " & _NumberFormat($g_aiHeroNeededResource[2]) & " Dark Elixir", $COLOR_INFO)
			If _Sleep(200) Then Return
			CloseWindow2()
		Case "Warden"
			Click($bXcoords[3] + 115, 232 + $g_iMidOffsetY)
			...
			SetLog($bHero & " Upgrade Requires " & _NumberFormat($g_aiHeroNeededResource[3]) & " Elixir", $COLOR_INFO)
			If _Sleep(200) Then Return
			CloseWindow2()
		Case "Champion"
			Click($bXcoords[4] + 115, 232 + $g_iMidOffsetY)
			...
			SetLog($bHero & " Upgrade Requires " & _NumberFormat($g_aiHeroNeededResource[4]) & " Dark Elixir", $COLOR_INFO)
			If _Sleep(200) Then Return
			CloseWindow2()
		Case "Duke"
			ClickDrag(400, 300 + $g_iMidOffsetY, 247, 300 + $g_iMidOffsetY, 50) ; Swipe left 153px to reveal Dragon Duke
			...
			Click($bXcoords[4] + 115, 232 + $g_iMidOffsetY)
			If _Sleep($DELAYUPGRADEHERO3) Then Return ; Wait for window to open
			$g_iDragonDukeLevel = getYellowLevel(452, 116)
			$g_aiHeroNeededResource[5] = Number(getCostsUpgradeRed(552, 541 + $g_iMidOffsetY)) ;read Red upgrade text
			If $g_aiHeroNeededResource[5] = "" Then $g_aiHeroNeededResource[5] = Number(getCostsUpgradeRed(552, 532 + $g_iMidOffsetY)) ;read Red upgrade text (Discount).
			SetLog($bHero & " Upgrade Requires " & _NumberFormat($g_aiHeroNeededResource[5]) & " Dark Elixir", $COLOR_INFO)
			If _Sleep(200) Then Return
			CloseWindow2()
...
Func UpgradeHeroes()
	...
	If Not $g_bUpgradeKingEnable And Not $g_bUpgradeQueenEnable And Not $g_bUpgradePrinceEnable And Not $g_bUpgradeWardenEnable And Not $g_bUpgradeChampionEnable And Not $g_bUpgradeDragonDukeEnable Then Return
	If _Sleep(500) Then Return

	If $g_iTownHallLevel < 7 Then
		SetLog("Town Hall Level " & $g_iTownHallLevel & " has no Hero Hall - Skip Locating", $COLOR_DEBUG)
		Return
	EndIf
	...
	; Ignore restart flag for hero upgrades to prevent blocking
	...
	If Not $bCheckValuesForWarden Then
		...
		If $g_bUpgradeChampionEnable Then
			...
			EndIf
		EndIf
		If $g_bUpgradeDragonDukeEnable Then
			If _DateIsValid($g_aiHeroUpgradeFinishDate[5]) Then
				If _DateDiff('n', _NowCalc(), $g_aiHeroUpgradeFinishDate[5]) < 0 Then
					$IsToOpen += 1
				Else
					If BitAND($g_iHeroUpgradingBit, $eHeroDragonDuke) <> $eHeroDragonDuke Then
						$IsToOpen += 1
					Else
						SetLog("Dragon Duke Upgrade Finishes @ " & $g_aiHeroUpgradeFinishDate[5] & ", Skip", $COLOR_INFO)
					EndIf
				EndIf
			Else
				If $g_aiHeroNeededResource[5] = 0 Then
					$IsToOpen += 1
				Else
					If Number($g_aiHeroNeededResource[5]) <= Number($g_aiCurrentLoot[$eLootDarkElixir]) Then
						$IsToOpen += 1
					Else
						SetLog("Dragon Duke Upgrade Requires " & _NumberFormat($g_aiHeroNeededResource[5]) & " Dark Elixir, Skip", $COLOR_INFO)
					EndIf
				EndIf
			EndIf
		EndIf
	...
	If BitOR($g_bUpgradeKingEnable, $g_bUpgradeQueenEnable, $g_bUpgradePrinceEnable, $g_bUpgradeWardenEnable, $g_bUpgradeChampionEnable, $g_bUpgradeDragonDukeEnable) Then
	...
	Switch $g_aiCmbCustomHeroOrder[4]
		Case 0
			If _IsHeroUpgrading($bXcoords[0], 429) Then
				...
				SetLog($g_asHeroNames[0] & " Status: Upgrading", $COLOR_DEBUG)
				;Set Status Variable
			...
			EndIf
		Case 1
			If _IsHeroUpgrading($bXcoords[1], 429) Then
				...
				SetLog($g_asHeroNames[1] & " Status: Upgrading", $COLOR_DEBUG)
				;Set Status Variable
			...
			EndIf
		Case 2
			If _IsHeroUpgrading($bXcoords[2], 429) Then
				...
				SetLog($g_asHeroNames[2] & " Status: Upgrading", $COLOR_DEBUG)
				;Set Status Variable
			...
			EndIf
		Case 3
			If _IsHeroUpgrading($bXcoords[3], 429) Then
				...
				SetLog($g_asHeroNames[3] & " Status: Upgrading", $COLOR_DEBUG)
				;Set Status Variable
			...
			EndIf
		Case 4
			If _IsHeroUpgrading($bXcoords[4], 429) Then
				...
				SetLog($g_asHeroNames[4] & " Status: Upgrading", $COLOR_DEBUG)
				;Set Status Variable
		...
		Case 5
			ClickDrag(400, 300 + $g_iMidOffsetY, 247, 300 + $g_iMidOffsetY, 50) ; Swipe left 153px to reveal Dragon Duke
			If _Sleep(800) Then Return ; Wait for scroll animation
			If _IsHeroUpgrading($bXcoords[5], 429) Then
				...
				SetLog($g_asHeroNames[5] & " Status: Upgrading", $COLOR_DEBUG)
				;Set Status Variable
			...
			Else
				...
				If ($g_abAttackTypeEnable[$DB] And BitAND($g_aiAttackUseHeroes[$DB], $g_aiSearchHeroWaitEnable[$DB], $eHeroDragonDuke) = $eHeroDragonDuke) Or _
						($g_abAttackTypeEnable[$LB] And BitAND($g_aiAttackUseHeroes[$LB], $g_aiSearchHeroWaitEnable[$LB], $eHeroDragonDuke) = $eHeroDragonDuke) Then                     ; check wait for hero status
					If $g_iSearchNotWaitHeroesEnable Then
	...
	;Check Hidden Hero Upgrade for Dragon Duke when it occupies slot 5 (last position, off-screen)
	If $g_aiCmbCustomHeroOrder[5] = 5 Then
		ClickDrag(400, 300 + $g_iMidOffsetY, 247, 300 + $g_iMidOffsetY, 50) ; Swipe left 153px to reveal Dragon Duke
		If _Sleep(800) Then Return ; Wait for scroll animation
		If _IsHeroUpgrading($bXcoords[5], 429) Then
			GUICtrlSetState($g_hPicDragonDukeGray, $GUI_HIDE)
	...
	; ### Dragon Duke ### (always last - requires scroll)
	If $g_bUpgradeDragonDukeEnable Then
		If BitAND($g_iHeroUpgradingBit, $eHeroDragonDuke) <> $eHeroDragonDuke Then
			If $g_iFreeBuilderCount < 1 + ($g_bAutoUpgradeWallsEnable And $g_bUpgradeWallSaveBuilder ? 1 : 0) Then
				SetLog("Not enough Builders available to upgrade the Dragon Duke")
				Return
			EndIf
			DukeUpgrade()
		ElseIf BitAND($g_iHeroUpgradingBit, $eHeroDragonDuke) = $eHeroDragonDuke Then
			If Not _DateIsValid($g_aiHeroUpgradeFinishDate[5]) Then FinishTimeCalculation("Duke")
		EndIf
	EndIf
...
Func KingUpgrade()
	...
	If $g_iTownHallLevel < 7 Then
		SetLog("Town Hall Upgrade Needed - Skipped!", $COLOR_DEBUG)
	...
	If _IsHeroUpgrading($bXcoords[0], 429) Then
		SetLog("King Upgrade: Running", $COLOR_INFO)
		Return
	EndIf
	Local $bOffset = False
	...
	Else
		If IsArray(_PixelSearch($bXcoords[0], 438 + $g_iMidOffsetY, $bXcoords[0] + 95, 444 + $g_iMidOffsetY, Hex(0xFF887F, 6), 20, True)) Then
			SetLog("Dark Elixir Insufficient", $COLOR_DEBUG)
		...
		If IsArray($HeroMaxLevel) And UBound($HeroMaxLevel) = 2 Then
			SetLog("Barbarian King is at max level, cannot upgrade anymore!", $COLOR_INFO)
			$g_bUpgradeKingEnable = False ; turn Off the King's upgrade
			GUICtrlSetState($g_hChkUpgradeKing, $GUI_UNCHECKED)
			chkUpgradeKing()
			ReducecmbHeroReservedBuilder()
		...
		If _ColorCheck(_GetPixelColor($bXcoords[0], 438 + $g_iMidOffsetY, True), Hex(0xADADAD, 6), 15) Or _ColorCheck(_GetPixelColor($bXcoords[0] + 42, 438 + $g_iMidOffsetY, True), Hex(0xADADAD, 6), 15) Then
			SetLog("Hero Hall Upgrade Needed - Skipped!", $COLOR_DEBUG)
			Return
		EndIf
	...
	If _ColorCheck(_GetPixelColor(800, 88 + $g_iMidOffsetY, True), Hex(0xF38E8D, 6), 20) Then ; Check if the Hero Upgrade window is open
		...
		If $g_iKingLevel = -1 Then
			...
			Return
		Else
			SetLog("Barbarian King Level: " & $g_iKingLevel, $COLOR_INFO)
		EndIf
	EndIf
	...
	If IsArray($RedSearch) Then
		If $g_aiCurrentLoot[$eLootDarkElixir] < ($g_afKingUpgCost[$g_iKingLevel] * 1000) * (1 - Number($g_iBuilderBoostDiscount) / 100) + $g_iUpgradeMinDark Then
			SetLog("Dark Elixir Insufficient, Requires:" & ($g_afKingUpgCost[$g_iKingLevel] * 1000) * (1 - Number($g_iBuilderBoostDiscount) / 100) & " + " & $g_iUpgradeMinDark, $COLOR_INFO)
			CloseWindow2(1)
			Return
		EndIf
	EndIf
	...
	Local $TxtWarningTip = GetTranslatedFileIni("MBR GUI Design Child Attack - Search", "TxtKingWait_Info_03", "ATTENTION: King auto upgrade is currently enabled")
	If (IsArray($aWhiteZeros) And UBound($aWhiteZeros, 1) = 2) Then
		...
		ClickP($aWhiteZeros, 1, 120)         ; Click upgrade button
	...
	ElseIf IsArray($YellowSearch) Then
		...
		ClickP($YellowSearch, 1, 120)         ; Click upgrade button
...
Func QueenUpgrade()
	...
	If _IsHeroUpgrading($bXcoords[1], 429) Then
		SetLog("Queen Upgrade: Running", $COLOR_INFO)
		Return
	EndIf
	Local $bOffset = False
	...
	Else
		...
		If IsArray($HeroMaxLevel) And UBound($HeroMaxLevel) = 2 Then
			SetLog("Archer Queen is at max level, cannot upgrade anymore!", $COLOR_INFO)
	...
	If _ColorCheck(_GetPixelColor(800, 88 + $g_iMidOffsetY, True), Hex(0xF38E8D, 6), 20) Then ; Check if the Hero Upgrade window is open
		...
		If $g_iQueenLevel = -1 Then
			...
			Return
		Else
			SetLog("Archer Queen Level: " & $g_iQueenLevel, $COLOR_INFO)
		EndIf
	EndIf
	...
	If IsArray($RedSearch) Then
		If $g_aiCurrentLoot[$eLootDarkElixir] < ($g_afQueenUpgCost[$g_iQueenLevel] * 1000) * (1 - Number($g_iBuilderBoostDiscount) / 100) + $g_iUpgradeMinDark Then
			SetLog("Dark Elixir Insufficient, Requires:" & ($g_afQueenUpgCost[$g_iQueenLevel] * 1000) * (1 - Number($g_iBuilderBoostDiscount) / 100) & " + " & $g_iUpgradeMinDark, $COLOR_INFO)
			CloseWindow2(1)
			Return
		EndIf
	EndIf
	...
	Local $TxtWarningTip = GetTranslatedFileIni("MBR GUI Design Child Attack - Search", "TxtQueenWait_Info_03", "ATTENTION: Queen auto upgrade is currently enabled")
	If (IsArray($aWhiteZeros) And UBound($aWhiteZeros, 1) = 2) Then
...
Func PrinceUpgrade()
	...
	SetLog("Upgrade Prince", $COLOR_ACTION)
	...
	If _IsHeroUpgrading($bXcoords[2], 429) Then
		SetLog("Prince Upgrade: Running", $COLOR_INFO)
		Return
	EndIf
	Local $bOffset = False
	...
	Else
		...
		If IsArray($HeroMaxLevel) And UBound($HeroMaxLevel) = 2 Then
			SetLog("Minion Prince is at max level, cannot upgrade anymore!", $COLOR_INFO)
			$g_bUpgradePrinceEnable = False ; turn Off the Princes upgrade
			GUICtrlSetState($g_hChkUpgradePrince, $GUI_UNCHECKED)
			chkUpgradePrince()
			ReducecmbHeroReservedBuilder()
	...
	If _ColorCheck(_GetPixelColor(800, 88 + $g_iMidOffsetY, True), Hex(0xF38E8D, 6), 20) Then ; Check if the Hero Upgrade window is open
		...
		If $g_iPrinceLevel = -1 Then
			...
			Return
		Else
			SetLog("Minion Prince Level: " & $g_iPrinceLevel, $COLOR_INFO)
		EndIf
	EndIf
	...
	If IsArray($RedSearch) Then
		If $g_aiCurrentLoot[$eLootDarkElixir] < ($g_afPrinceUpgCost[$g_iPrinceLevel] * 1000) * (1 - Number($g_iBuilderBoostDiscount) / 100) + $g_iUpgradeMinDark Then
			SetLog("Dark Elixir Insufficient, Requires:" & ($g_afPrinceUpgCost[$g_iPrinceLevel] * 1000) * (1 - Number($g_iBuilderBoostDiscount) / 100) & " + " & $g_iUpgradeMinDark, $COLOR_INFO)
			CloseWindow2(1)
			Return
		EndIf
	EndIf
	...
	Local $TxtWarningTip = GetTranslatedFileIni("MBR GUI Design Child Attack - Search", "TxtPrinceWait_Info_03", "ATTENTION: Prince auto upgrade is currently enabled")
	If (IsArray($aWhiteZeros) And UBound($aWhiteZeros, 1) = 2) Then
...
Func WardenUpgrade()
	...
	If Not $bCheckValuesForWarden Then
		SetLog("Upgrade Warden", $COLOR_ACTION)
	EndIf
	...
	If _IsHeroUpgrading($bXcoords[3], 429) Then
		SetLog("Warden Upgrade: Running", $COLOR_INFO)
		Return
	EndIf
	Local $bOffset = False
	...
	Else
		If IsArray(_PixelSearch($bXcoords[3], 438 + $g_iMidOffsetY, $bXcoords[3] + 95, 444 + $g_iMidOffsetY, Hex(0xFF887F, 6), 20, True)) Then
			SetLog("Elixir Insufficient", $COLOR_DEBUG)
		...
		If IsArray($HeroMaxLevel) And UBound($HeroMaxLevel) = 2 Then
			SetLog("Grand Warden is at max level, cannot upgrade anymore!", $COLOR_INFO)
			$g_bUpgradeWardenEnable = False ; turn Off the Wardens upgrade
			GUICtrlSetState($g_hChkUpgradeWarden, $GUI_UNCHECKED)
			chkUpgradeWarden()
			ReducecmbHeroReservedBuilder()
	...
	If _ColorCheck(_GetPixelColor(800, 88 + $g_iMidOffsetY, True), Hex(0xF38E8D, 6), 20) Then ; Check if the Hero Upgrade window is open
		...
		If $g_iWardenLevel = -1 Then
			...
			Return
		Else
			SetLog("Grand Warden Level: " & $g_iWardenLevel, $COLOR_INFO)
		EndIf
	EndIf

	If $bCheckValuesForWarden Then
		If $g_aiHeroNeededResource[3] = 0 Then
			If $g_iWardenLevel < 0 Then
				SetLog("Unable To Read Warden Level For Cost Calculation, Skip!", $COLOR_ERROR)
				CloseWindow2(1)
				Return
			EndIf
			$g_aiHeroNeededResource[3] = ($g_afWardenUpgCost[$g_iWardenLevel] * 1000000 * (1 - Number($g_iBuilderBoostDiscount) / 100))
			SetLog("Warden Upgrade Requires " & _NumberFormat($g_aiHeroNeededResource[3]) & " Elixir", $COLOR_INFO)
			CloseWindow2(1)
			Return
		EndIf
	EndIf
	...
	If IsArray($RedSearch) Then
		If $g_aiCurrentLoot[$eLootElixir] < ($g_afWardenUpgCost[$g_iWardenLevel] * 1000000) * (1 - Number($g_iBuilderBoostDiscount) / 100) + $g_iUpgradeMinDark Then
			SetLog("Elixir Insufficient, Requires: " & ($g_afWardenUpgCost[$g_iWardenLevel] * 1000000) * (1 - Number($g_iBuilderBoostDiscount) / 100) & " + " & $g_iUpgradeMinDark, $COLOR_INFO)
			CloseWindow2(1)
			Return
		EndIf
	EndIf
	...
	Local $TxtWarningTip = GetTranslatedFileIni("MBR GUI Design Child Attack - Search", "TxtWardenWait_Info_03", "ATTENTION: Warden auto upgrade is currently enabled")
	If (IsArray($aWhiteZeros) And UBound($aWhiteZeros, 1) = 2) Then
...
Func ChampionUpgrade()
	...
	If _IsHeroUpgrading($bXcoords[4], 429) Then
		SetLog("Champion Upgrade: Running", $COLOR_INFO)
		Return
	EndIf
	Local $bOffset = False
	...
	Else
		...
		If IsArray($HeroMaxLevel) And UBound($HeroMaxLevel) = 2 Then
			SetLog("Royal Champion is at max level, cannot upgrade anymore!", $COLOR_INFO)
			$g_bUpgradeChampionEnable = False ; turn Off the Champions upgrade
			GUICtrlSetState($g_hChkUpgradeChampion, $GUI_UNCHECKED)
			chkUpgradeChampion()
			ReducecmbHeroReservedBuilder()
	...
	If _ColorCheck(_GetPixelColor(800, 88 + $g_iMidOffsetY, True), Hex(0xF38E8D, 6), 20) Then ; Check if the Hero Upgrade window is open
		...
		If $g_iChampionLevel = -1 Then
			...
			Return
		Else
			SetLog("Royal Champion Level: " & $g_iChampionLevel, $COLOR_INFO)
		EndIf
	EndIf
	...
	If IsArray($RedSearch) Then
		If $g_aiCurrentLoot[$eLootDarkElixir] < ($g_afChampionUpgCost[$g_iChampionLevel] * 1000) * (1 - Number($g_iBuilderBoostDiscount) / 100) + $g_iUpgradeMinDark Then
			SetLog("Dark Elixir Insufficient, Requires:" & ($g_afChampionUpgCost[$g_iChampionLevel] * 1000) * (1 - Number($g_iBuilderBoostDiscount) / 100) & " + " & $g_iUpgradeMinDark, $COLOR_INFO)
			CloseWindow2(1)
			Return
		EndIf
	EndIf
	...
	Local $TxtWarningTip = GetTranslatedFileIni("MBR GUI Design Child Attack - Search", "TxtChampionWait_Info_03", "ATTENTION: Champion auto upgrade is currently enabled")
	If (IsArray($aWhiteZeros) And UBound($aWhiteZeros, 1) = 2) Then
...
Func DukeUpgrade()
	...
	If Not $g_bUpgradeDragonDukeEnable Then Return

	SetLog("Upgrade Dragon Duke", $COLOR_ACTION)

	If Not $g_bRunState Then Return

	If $g_iTownHallLevel < 15 Then
		SetLog("Town Hall Upgrade Needed - Skipped!", $COLOR_DEBUG)
		$g_bUpgradeDragonDukeEnable = False ; turn Off Dragon Duke upgrade
		GUICtrlSetState($g_hChkUpgradeDragonDuke, $GUI_UNCHECKED)
		chkUpgradeDragonDuke()
		ReducecmbHeroReservedBuilder()
		Return
	EndIf

	; Scroll right to reveal Dragon Duke (off-screen, 6th hero slot)
	ClickDrag(400, 300 + $g_iMidOffsetY, 247, 300 + $g_iMidOffsetY, 50) ; Swipe left 153px to reveal Dragon Duke
	If _Sleep(800) Then Return ; Wait for scroll animation

	If _IsHeroUpgrading($bXcoords[5], 429) Then
		SetLog("Dragon Duke Upgrade: Running", $COLOR_INFO)
		If _Sleep(500) Then Return
		Return
	EndIf
	Local $bOffset = False
	...
	Else
		If IsArray(_PixelSearch($bXcoords[4], 438 + $g_iMidOffsetY, $bXcoords[4] + 95, 444 + $g_iMidOffsetY, Hex(0xFF887F, 6), 20, True)) Then
			SetLog("Dark Elixir Insufficient", $COLOR_DEBUG)
			If $g_aiHeroNeededResource[5] = 0 Then NeededResources("Duke")
			Return
		EndIf
		Local $HeroMaxLevel = decodeSingleCoord(FindImageInPlace2("HeroMaxLevel", $ImgHeroMaxLevel, $bXcoords[4] + 20, 420 + $g_iMidOffsetY, $bXcoords[4] + 100, 445 + $g_iMidOffsetY, True))
		If IsArray($HeroMaxLevel) And UBound($HeroMaxLevel) = 2 Then
			SetLog("Dragon Duke is at max level, cannot upgrade anymore!", $COLOR_INFO)
			$g_bUpgradeDragonDukeEnable = False ; turn Off Dragon Duke upgrade
			GUICtrlSetState($g_hChkUpgradeDragonDuke, $GUI_UNCHECKED)
			chkUpgradeDragonDuke()
			ReducecmbHeroReservedBuilder()
	...
	If $bOffset Then
		Local $g_iDukeCostOCR = getOcrAndCapture("coc-HeroCost", $bXcoords[4] + 38, 433 + $g_iMidOffsetY, 65, 20, True)
	Else
		Local $g_iDukeCostOCR = getOcrAndCapture("coc-HeroCost", $bXcoords[4] + 23, 433 + $g_iMidOffsetY, 65, 20, True)
	EndIf
	If $g_iDukeCostOCR = "" Then
		SetLog("Dragon Duke Cost OCR Read Error", $COLOR_ERROR)
		Return
	EndIf
	...
	If $g_bDebugImageSave Then SaveDebugImage("UpgradeDarkBtn1")
	If _ColorCheck(_GetPixelColor(800, 88 + $g_iMidOffsetY, True), Hex(0xF38E8D, 6), 20) Then ; Check if the Hero Upgrade window is open
		Local $g_iDukeLevelOCR = getYellowLevel(525, 116)
		$g_iDragonDukeLevel = Number($g_iDukeLevelOCR - 1)
		If $g_iDragonDukeLevel = -1 Then
			SetLog("Unable To Read Dragon Duke Level, Skip Upgrade!", $COLOR_ERROR)
			CloseWindow2(1)
			Return
		Else
			SetLog("Dragon Duke Level: " & $g_iDragonDukeLevel, $COLOR_INFO)
		EndIf
	EndIf

	Local $RedSearch = _PixelSearch(610, 535 + $g_iMidOffsetY, 650, 557 + $g_iMidOffsetY, Hex(0xFF887F, 6), 20)
	If IsArray($RedSearch) Then
		If $g_aiCurrentLoot[$eLootDarkElixir] < ($g_afDragonDukeUpgCost[$g_iDragonDukeLevel] * 1000) * (1 - Number($g_iBuilderBoostDiscount) / 100) + $g_iUpgradeMinDark Then
			SetLog("Dark Elixir Insufficient, Requires:" & ($g_afDragonDukeUpgCost[$g_iDragonDukeLevel] * 1000) * (1 - Number($g_iBuilderBoostDiscount) / 100) & " + " & $g_iUpgradeMinDark, $COLOR_INFO)
			CloseWindow2(1)
			Return
		EndIf
	EndIf
	...
	If (IsArray($aWhiteZeros) And UBound($aWhiteZeros, 1) = 2) Then
		...
		Local $bUpgradeCost = ($g_afDragonDukeUpgCost[$g_iDragonDukeLevel] * 1000 * (1 - Number($g_iBuilderBoostDiscount) / 100))
		SetLog("Upgrade Cost " & _NumberFormat($bUpgradeCost) & " Dark Elixir", $COLOR_INFO)
		If _Sleep(1000) Then Return
		...
		If $g_bDebugImageSave Then SaveDebugImage("UpgradeDarkBtn2")
		If isGemOpen(True) Then         ; Redundant Safety Check if the use Gem window opens
			SetLog("Dragon Duke Upgrade Fail! Gem Window popped up!", $COLOR_ERROR)
			Return
		EndIf
		SetLog("Dragon Duke Upgrade Complete", $COLOR_SUCCESS)
		GUICtrlSetState($g_hPicDragonDukeGray, $GUI_HIDE)
		GUICtrlSetState($g_hPicDragonDukeRed, $GUI_SHOW)
		GUICtrlSetState($g_hPicDragonDukeBlue, $GUI_HIDE)
		GUICtrlSetState($g_hPicDragonDukeGreen, $GUI_HIDE)
		...
		$g_iCostDElixirHero += $bUpgradeCost
		UpdateStats()
		If Not $g_bRepUpgradeDragonDukeEnable Then
			$g_bUpgradeDragonDukeEnable = False ; turn Off Dragon Duke upgrade
			GUICtrlSetState($g_hChkUpgradeDragonDuke, $GUI_UNCHECKED)
			_GUI_Value_STATE("HIDE", $groupDragonDukeSleeping)
			GUICtrlSetState($g_hChkRepUpgradeDragonDuke, $GUI_UNCHECKED + $GUI_DISABLE)
		EndIf
		Local $bFinishTime = ConvertOCRTime("FinishTime", $g_aUpgradeDuration, False)
		$g_aiHeroUpgradeFinishDate[5] = _DateAdd('n', Ceiling($bFinishTime), _NowCalc())
		SetLog("Dragon Duke Upgrade Finishes @ " & $g_aiHeroUpgradeFinishDate[5], $COLOR_SUCCESS)
		$g_aiHeroNeededResource[5] = 0
		$g_aiCurrentLoot[$eLootDarkElixir] -= $bUpgradeCost
	ElseIf IsArray($YellowSearch) Then
		...
		$g_aiCurrentLoot[$eLootDarkElixir] -= $bUpgradeCost
	Else
		SetLog("Dragon Duke Upgrade Fail! No DE!", $COLOR_ERROR)
		CloseWindow2(1)
		Return
	EndIf
...
Func ReservedBuildersForHeroes($aSetLog = True)
	Local $iUsedBuildersForHeroes = Number(BitAND($g_iHeroUpgradingBit, $eHeroKing) = $eHeroKing ? 1 : 0) + Number(BitAND($g_iHeroUpgradingBit, $eHeroQueen) = $eHeroQueen ? 1 : 0) + _
			Number(BitAND($g_iHeroUpgradingBit, $eHeroPrince) = $eHeroPrince ? 1 : 0) + Number(BitAND($g_iHeroUpgradingBit, $eHeroWarden) = $eHeroWarden ? 1 : 0) + Number(BitAND($g_iHeroUpgradingBit, $eHeroChampion) = $eHeroChampion ? 1 : 0) + _
			Number(BitAND($g_iHeroUpgradingBit, $eHeroDragonDuke) = $eHeroDragonDuke ? 1 : 0)
	If $aSetLog Then SetLog("Builders upgrading heroes: " & $iUsedBuildersForHeroes)

	Local $iFreeBuildersReservedForHeroes = _Max(Number($g_iHeroReservedBuilder) - $iUsedBuildersForHeroes, 0)
	If $aSetLog Then SetLog("Free builders reserved for heroes: " & $iFreeBuildersReservedForHeroes)
