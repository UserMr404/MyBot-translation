...
Func InitAndroidConfig($bRestart = False)
	...
	If $bRestart = False Then
		If UBound($g_avAndroidAppConfig, 0) <> 2 Then
			SetLog("Error: $g_avAndroidAppConfig is not 2D", $COLOR_ERROR)
			$g_iAndroidConfig = 0
		EndIf
		If $g_iAndroidConfig < 0 Or $g_iAndroidConfig >= UBound($g_avAndroidAppConfig) Then
			$g_iAndroidConfig = 0
		EndIf
...
Func WinGetAndroidHandle($bInitAndroid = Default, $bTestPid = False)
	...
	If IsHWnd($g_hAndroidWindow) = 1 Then
		...
		If $currHWnD = 0 Or $currHWnD <> $g_hAndroidWindow Then
			...
			SetLog($g_sAndroidEmulator & $instance & " is running in window mode", $COLOR_ACTION)
...
Func _RestartAndroidCoC($bInitAndroid = True, $bRestart = True, $bStopCoC = True, $iRetry = 0, $iRecursive = 0)
	...
	If $bRestart = True Then
		If $bStopCoC Then
			SetLog("Please wait for Clash of Clans restart", $COLOR_INFO) ; Let user know we need time...
			$sRestart = "-S "
		Else
			SetLog("Please wait for Clash of Clans restart", $COLOR_INFO) ; Let user know we need time...
		EndIf
	Else
		SetLog("Launching Clash of Clans!", $COLOR_SUCCESS)
	EndIf
	ConnectAndroidAdb()
	If Not $g_bRunState Then Return False
	;AndroidAdbTerminateShellInstance()
	...
	If GetAndroidProcessPID(Default, False) = 0 And @error = 0 Then
		...
		Else
			...
			EndIf
			$iRetry += 1
			SetLog("Unable to load Clash of Clans, retry " & $iRetry & "...", $COLOR_ERROR)
...
Func GetAndroidProcessPID($sPackage = Default, $bForeground = True, $iRetryCount = 0)
	...
	SetLog("Android process '" & $sPackage & "' is not running")
...
Func LaunchAndroid($sProgramPath, $sCmdParam, $sPath, $iWaitInSecAfterLaunch = Default, $bStopIfLaunchFails = True)
	...
	If Not $g_sAndroidPicturesHostPath Then
		SetScreenAndroid()
	EndIf
	SetLog("Please wait while " & $g_sAndroidEmulator & " and Clash of Clans start", $COLOR_SUCCESS)
	Local $pid = 0
...
Func UpdateAndroidBackgroundMode()
	...
	Else
		...
		Switch $iBackgroundMode
			...
			Case $g_iAndroidBackgroundModeVulkan
				$sGraphicsEngine = "Vulkan"
				SetDebugLog($g_sAndroidEmulator & " (" & $g_sAndroidInstance & ") is using Vulkan, disable WinAPI for Background Mode")
				AndroidSupportFeaturesRemove(1) ; disabled DirectX Background Mode
			Case Else
				; using default mode
	...
	Switch $iMode
		...
		Case 2 ; ADB screencap mode (slower, but alwasy works even if Monitor is off -> "True Brackground Mode")
			...
			Else
				...
				EndIf
			EndIf
		Case 3 ; Vulkan mode (similar to OpenGL, use ADB screencap)
			If $g_bAndroidAdbScreencapEnabled <> True Or $g_bAndroidSharedFolderAvailable <> True Then
				SetLog("Android ADB screencap disabled, please check Android Options", $COLOR_ERROR)
			Else
				If BitAND($g_iAndroidSupportFeature, 2) = 0 Then
					SetLog("Android ADB screencap not available, Vulkan Background Mode not supported", $COLOR_ERROR)
				Else
					; Ok, enable screencap
					SetDebugLog("Enable ADB screencap for Vulkan Background Mode")
					$g_bAndroidAdbScreencap = $g_bAndroidAdbScreencapEnabled = True And BitAND($g_iAndroidSupportFeature, 2) = 2 ; Use Android ADB to capture screenshots in RGBA raw format
				EndIf
			EndIf
		Case Else
...
Func CheckEmuNewVersions()
	...
	Switch $g_sAndroidEmulator
		Case "BlueStacks5"
			$NewVersion = GetVersionNormalized("5.22.125.1001")
		Case "MEmu"
