...
Func CloseCoC($ReOpenCoC = False, $bCheckRunState = True)
	...
	Local $Adb = ""
	If $ReOpenCoC Then
		SetLog("Please wait for Clash of Clans restart", $COLOR_ERROR) ; Let user know we need time...
	Else
		SetLog("Closing Clash of Clans...", $COLOR_ERROR) ; Let user know what we do...
	EndIf
	WinGetAndroidHandle()
	If $bCheckRunState And Not $g_bRunState Then Return FuncReturn()
...
Func _WaitnOpenCoC($iWaitTime, $bFullRestart = False, $bSuspendComputer = False, $bLockBotSlot = False)
	...
	SetLog("Waiting " & $sWaitTime & "before starting Clash of Clans", $COLOR_SUCCESS)
	ReduceBotMemory()
	...
	If $bPoliteCloseCoC Then
		...
		If $g_sAndroidGameDistributor = $g_sGoogle Then
			...
			While 1
				...
				If $i > 10 Then
					SetLog("Can not find Okay button to exit Clash of Clans, Forcefully Closing Clash of Clans", $COLOR_ERROR)
			...
			While 1
				...
				Switch $g_sAndroidGameDistributor
					...
					Case "Wandoujia/Downjoy", "Haimawan", "Leshi", "Tencent"
						ContinueCase
					Case Else
						SetLog("Polite Close Unsupported - " & $g_sAndroidGameDistributor & ", Forcefully Closing Clash of Clans", $COLOR_ERROR)
						If $g_bDebugImageSave Then SaveDebugImage($sSource)
						CloseCoC()
						ExitLoop
				EndSwitch
				If $i > 10 Then
					SetLog("Cannot find exit button: " & $g_sAndroidGameDistributor & ", Forcefully Closing Clash of Clans", $COLOR_ERROR)
					If $g_bDebugImageSave Then SaveDebugImage($sSource)
					CloseCoC()
					ExitLoop
				EndIf
