...
Func getAllEmulators()
	...
	If $sEmulatorString <> "" Then
		SetLog("Emulator" & (UBound($aEmulator) > 1 ? "s" : "") & " detected on your system:")
		...
		SetLog("  - " & $aEmulator[$i] & " v" & $emuVer, $COLOR_SUCCESS)
		Next
	Else
		SetLog("No emulator detected on your system", $COLOR_DEBUG)
		Return
	EndIf
