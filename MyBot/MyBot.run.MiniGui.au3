...
Func InitAndroid($bCheckOnly = False, $bLogChangesOnly = True)
	...
	If Not $bCheckOnly And $Result Then
		...
		; Auto hide Android if option is enabled
...
Func DirectAttack()
	; Disable both possible Direct Attack controls to prevent double click
	...
	SetLog("DirectAttack: Attack simulation started", $COLOR_INFO)
	SetLog("DirectAttack: Preparing attack", $COLOR_INFO)
	SetLog("DirectAttack: Executing attack", $COLOR_INFO)
	SetLog("DirectAttack: Attack completed!", $COLOR_SUCCESS)
	If $g_hBtnDirectAttack <> 0 Then GUICtrlSetState($g_hBtnDirectAttack, $GUI_ENABLE)
	If $g_hbtnAttNow <> 0 Then GUICtrlSetState($g_hbtnAttNow, $GUI_ENABLE)
EndFunc   ;==>DirectAttack

Func ToggleBBSpam()
	SetLog("ToggleBBSpam: BB Spam Toggle Called", $COLOR_INFO)
EndFunc   ;==>ToggleBBSpam

Func ToggleCCSpam()
	SetLog("ToggleCCSpam: CC Spam Toggle Called", $COLOR_INFO)
